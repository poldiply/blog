...
//   r->coeffs[1456] = ((a[2002] >> 0) | ((uint32_t)a[2003] << 8)) & 0x7FF;                             
//   r->coeffs[1457] = ((a[2003] >> 3) | ((uint32_t)a[2004] << 5)) & 0x7FF;        
...
void pack_pk(uint8_t pk[NCC_CRYPTO_PUBLICKEYBYTES],
...
void unpack_pk(uint8_t zeta[SEEDBYTES],
               poly *t1,
               const uint8_t pk[NCC_CRYPTO_PUBLICKEYBYTES])
{
  unsigned int i;
...
void pack_sk(uint8_t sk[NCC_CRYPTO_PUBLICKEYBYTES],
             const uint8_t zeta[SEEDBYTES],
...
void unpack_sk(uint8_t zeta[SEEDBYTES],
               ...
               poly *s1,
               poly *s2,
               const uint8_t sk[NCC_CRYPTO_PUBLICKEYBYTES])
{
  unsigned int i;
...
void pack_sig(uint8_t sig[NCC_CRYPTO_BYTES],
...
int unpack_sig(uint8_t c[SEEDBYTES],
               poly *z,
               poly *h,
               const uint8_t sig[NCC_CRYPTO_BYTES])
{
  unsigned int i;
