...
void pack_sk(uint8_t sk[HAETAE_CRYPTO_SECRETKEYBYTES],
             const uint8_t pk[HAETAE_CRYPTO_PUBLICKEYBYTES], const polyvecm *s0,
...
int pack_sig(uint8_t sig[HAETAE_CRYPTO_BYTES], const poly *c,
    ...
    if (SEEDBYTES + L * N + 2 + size_enc_hb_z1 + size_enc_h > HAETAE_CRYPTO_BYTES) {
...
int unpack_sig(poly *c, polyvecl *lowbits_z1,
               ...
               const uint8_t sig[HAETAE_CRYPTO_BYTES]) {
    ...
    if (HAETAE_CRYPTO_BYTES < (2 + L * N + SEEDBYTES + size_enc_h + size_enc_hb_z1))
    ...
    for (int i = 0; i < HAETAE_CRYPTO_BYTES - (SEEDBYTES + L * N + 2 + size_enc_hb_z1 +
                                        size_enc_h);
         i++)
        if (sig[i] != 0)
